...
queues_not_fully_az_covered() ->
    TagKey = application:get_env(rabbit, quorum_queue_member_placement_tag, <<"az">>),
    queues_not_fully_az_covered(TagKey).

-spec queues_not_fully_az_covered(binary()) -> [#{queue        := rabbit_types:r(queue),
                                                  member_nodes := [node()],
                                                  member_azs   := [binary() | undefined],
                                                  missing_azs  := [binary()]}].
queues_not_fully_az_covered(TagKey) ->
...
-spec member_distribution_per_az(binary()) -> #{binary() | undefined => #{node() => non_neg_integer()}}.
member_distribution_per_az(TagKey) ->
    Nodes = rabbit_nodes:list_running(),
    NodeToAZ = ?MODULE:node_tags_for_nodes(Nodes, TagKey),
...
%% Uses erpc:multicall for remote nodes and the local app env for the local node.
-spec node_tags_for_nodes([node()], binary()) -> #{node() => binary() | undefined}.
node_tags_for_nodes(Nodes, TagKey) ->
    LocalNode = ?MODULE:node(),
    LocalTags = application:get_env(rabbit, node_tags, []),
    LocalVal = proplists:get_value(TagKey, LocalTags, undefined),
    RemoteNodes = lists:delete(LocalNode, Nodes),
    Results = erpc:multicall(RemoteNodes, application, get_env, [rabbit, node_tags, []], 5000),
    RemoteVals = lists:map(fun({ok, Tags}) -> proplists:get_value(TagKey, Tags, undefined);
                              (_Error)     -> undefined
                           end, Results),
    RemoteMap = maps:from_list(lists:zip(RemoteNodes, RemoteVals)),
    RemoteMap#{LocalNode => LocalVal}.
